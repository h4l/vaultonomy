// Vendored from: https://github.com/h4l/anonystat/blob/main/payload_schemas.ts
...
export const UserProperties = z
  .record(UserPropertyName, z.object({ value: UserPropertyValue }))
...
export const AnyParameters = z.record(ParameterName, z.any()).pipe(
  ...
  z
    .object({ items: z.array(AnyItem).max(10).optional() })
    .catchall(ParameterValue),
);
...
const MicrosecondTimeStamp = z
  .string()
  .regex(nonNegDecimalIntegerPattern)
...
// type EventType<T extends

export type AnyEventSchema = z.AnyZodObject | z.ZodEffects<z.AnyZodObject>;

export type PayloadSchema<EventSchemaT extends AnyEventSchema> = z.ZodObject<{
  events: z.ZodArray<EventSchemaT>;
}>;
